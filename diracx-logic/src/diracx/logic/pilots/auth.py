...
async def create_raw_secrets(
    ...
    secrets_added = await get_secrets_by_hashed_secrets_bulk(
        pilot_db=pilot_db,
        hashed_secrets=hashed_secrets,
...
async def update_secrets_constraints(
    ...
    secrets_obj = await get_secrets_by_hashed_secrets_bulk(
        pilot_db=pilot_db,
...
async def verify_pilot_credentials(
    ...
    pilots = await get_pilots_by_stamp_bulk(
        ...
        # FIXME: Remove PilotStamp
        # See https://github.com/DIRACGrid/diracx/pull/570#discussion_r2149215887
    )
    pilot = dict(pilots[0])

    # 2. Get the secret itself
    secrets = await get_secrets_by_hashed_secrets_bulk(
...
async def refresh_pilot_token(
    ...
    pilots = await get_pilots_by_stamp_bulk(
        pilot_db=pilot_db, pilot_stamps=[pilot_stamp], parameters=["VO"]
    )
